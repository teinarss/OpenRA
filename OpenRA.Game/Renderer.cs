...
namespace OpenRA
{
	public sealed class Renderer : IDisposable
	{
		...
		public void BeginFrame(int2 scroll, float zoom)
		{
			...
			// Cache property results to avoid the unnecessary overhead of repeat queries
			var resolution = Resolution;
			var scale = WindowScale;

			var backingResolution = new Size((int)(scale * resolution.Width), (int)(scale * resolution.Height));
			var backingBufferSize = backingResolution.NextPowerOf2();
			if (screenSprite == null || backingBufferSize != screenSprite.Sheet.Size)
			{
				if (screenBuffer != null)
					screenBuffer.Dispose();

				// Render into a frame buffer to support render != window sizes
				// and reading data back for screenshots
				screenBuffer = Context.CreateFrameBuffer(backingBufferSize);
				var screenSheet = new Sheet(SheetType.BGRA, screenBuffer.Texture);

				// Flip sprite in Y to match OpenGL's bottom-left origin
				var screenBounds = Rectangle.FromLTRB(0, backingResolution.Height, backingResolution.Width, 0);
		...
		{
			var bufferSize = Resolution.NextPowerOf2();
